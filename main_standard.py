...
from losses.shot import CrossEntropyLabelSmooth
...
def add_parser_argument(parser):
    ...
    parser.add_argument('--attack', type=str, choices=['fast', 'pgd', 'trades'])
...
def main():
    ...
    if opt.target_dataset:
        trainloader, raw_trainloader, _ = dataset.get_loader(
            opt.data_dir, opt.batch, opt.num_workers, with_index=True)
        target_dataset = get_dataset(opt.target_dataset)
        testloader = target_dataset.get_loader(
            opt.data_dir, opt.batch, opt.num_workers, with_index=True,
            train_split=opt.train_split, val_split=opt.val_split,
            split_seed=opt.data_split_seed)[-1]
    else:
    ...
    if opt.pretrained or opt.load_run_name:
        ...
        parameters = model.parameters()
    if opt.arch_variant == 'shot' and opt.target_dataset is None:
        logger.info(f'==> Remove WeightNorm')
